...
main()
{
	...
	/* Counters for every transmission differents for each pair of nodes */
	int counter12_a, counter12_b, counter21_a, counter21_b, counter31_a, counter31_c, counter13_a, counter13_c,  counter23_b, counter23_c, counter32_b, counter32_c;
	
	counter12_a = counter12_b = 4;
	counter13_a = counter13_c =9;
	counter23_b = counter23_c = 12;
	counter21_a = counter21_b = 11;
	counter31_a = counter31_c = 19;
	counter32_b = counter32_c = 5;
	/* a unique code for every transmission */
	char code12 = "a";
	char code21 = "b";
	char code13 = "c";
	char code31 = "d";
	...
	printf("Node1 sends message to node2.\n");
	sha3_Init256(&w);
	sha3_Update(&w, "114a", 4);
	val = sha3_Finalize(&w);
	printf("Broadcasts: ID[ %d ]\n",*val);
	counter12_a++;

	printf("\nNode2 checks if id is from node1, or node3\n");
	sha3_Init256(&b);
	sha3_Update(&b, "114f",4);
	val2 = sha3_Finalize(&b);
	printf("Calculated id from node3: %d \n",*val2);	
	sha3_Init256(&b);
	sha3_Update(&b, "114a",4);
	val2 = sha3_Finalize(&b);
	printf("Calculated id from node1: %d \n",*val2);	
	printf("Verified message from node1\n");
	counter12_b++;

	printf("\nNode3 checks if message is from node1 or node 2\n");
	sha3_Init256(&b);
	sha3_Update(&b, "119c",4);
	val2 = sha3_Finalize(&b);
	printf("Calculated id from node1: %d \n",*val2);	
	sha3_Init256(&b);
	sha3_Update(&b, "4512e",5);
	val2 = sha3_Finalize(&b);
	printf("Calculated id from node2: %d \n",*val2);	
	printf("Rejects, message not for node3\n");


	printf("\nNode1 sends message to node3.\n");
	sha3_Init256(&w);
	sha3_Update(&w, "119c", 4);
	val = sha3_Finalize(&w);
	printf("Broadcasts: ID[ %d ]\n",*val);
	counter13_a++;
	...
	printf("\nNode2 checks if id is from node1, or node3\n");
	sha3_Init256(&b);
	sha3_Update(&b, "455f",4);
	val2 = sha3_Finalize(&b);
	printf("Calculated id from node3: %d \n",*val2);	
	sha3_Init256(&b);
	sha3_Update(&b, "115a",4);
	val2 = sha3_Finalize(&b);
	printf("Calculated id from node1: %d \n",*val2);	
	printf("Rejects, message not for node2\n");
