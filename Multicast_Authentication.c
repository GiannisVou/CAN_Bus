...
/*	CAN Bus 9 nodes	
	Explanation of schema on README.
...
void * node_two(void *unused)
{
	...
	while(1) {
			...
			if(frame.id == 447) {
				...
				if(*hash == frame.mac2) {
					printf("\tAccepts message from node5\n");
					accepted++;
					counter5++;
				}	
			}
...
void * node_five(void *unused)
{
	...
	while(1) {
			...
			counter5++;	// incrementing counter after transmission.			
			print_CAN_frame();
			...
			printf("\t\tNode5 listening.. id: %X :: ",id);	
			...
			// no node adds a MAC3,MAC4 for node4.

			if( !accepted )
				printf("\tNode5 rejects message\n");
...
void * node_six(void *unused)
{
	...
	while(1) {
			...
			counter6++;	// incrementing counter after transmission.			
			print_CAN_frame();
			...
			printf("\t\tNode6 listening.. id: %X :: ",id);	
...
void * node_seven(void *unused)
{
	...
	while(1) {
			...
			counter7++;	// incrementing counter after transmission.			
			print_CAN_frame();
			...
			printf("\t\tNode7 listening.. id: %X :: ",id);	
...
void *node_eight(void *unused)
{
	...
	while(1) {
			...
			counter8++;	// incrementing counter after transmission.			
			print_CAN_frame();
			...
			printf("\t\tNode8 listening.. id: %X :: ",id);	
...
void * node_nine(void *unused)
{
	...
	while(1) {
			...
			counter9++;	// incrementing counter after transmission.			
			print_CAN_frame();
			...
			printf("\t\tNode9 listening.. id: %X :: ",id);	
...
int main(int argc,char *argv[])
{
	...
	pthread_barrier_init(&bar, NULL,9);
	pthread_barrier_init(&bar2, NULL,9);
	pthread_barrier_init(&listen, NULL,9);
